...
- **Default Triggers**:
  ...
  The current default triggers are `/oc` and `/opencode`.
- **Scope**:
  The OpenCode action in this directory is intended to be a generic wrapper for OpenCode and should remain decoupled from specialized Cogni AI agent triggers (like `/brainstorm` or `/review`) unless explicitly requested.
